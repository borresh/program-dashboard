...
You are Agent B working on the `program-dashboard` project inside an isolated Docker Sandbox.

## Scope

- Work **only** in this clone (the in-sandbox workspace at `.`).
- Your branch is `agent/b`. Commit all work there.
- Never touch `main`, `agent/a`, or any other branch.
